...
func (h *Inbound) UpdateUsers(users []option.TrojanUser) error {
	...
	h.users = users
	userList, passwordList := h.assignUserIDs(users)
	return h.service.UpdateUsers(userList, passwordList)
}
...
func (h *Inbound) assignUserIDs(users []option.TrojanUser) ([]int, []string) {
